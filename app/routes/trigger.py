...
from schemas.trigger import TriggerBase, TriggerRead, TriggerUpdate, CloneRequest
...
def create_trigger(
    ...
    return crud_trigger.create_trigger(db=db, trigger=trigger, user_id=current_user.id)
...
def update_trigger(
    trigger_id: int, trigger_update: TriggerUpdate, db: Session = Depends(get_db)
):
