...
from schemas.reaction import ReactionBase, ReactionUpdate, CloneRequest


def create_reaction(db: Session, reaction: ReactionBase, user_id: int):
    db_reaction = Reaction(name=reaction.name, user_id=user_id)
    db.add(db_reaction)
    db.commit()
...
def get_reaction(db: Session, user_id: int):
    return (
        db.query(Reaction)
        .filter((Reaction.user_id == user_id) | (Reaction.user_id == None))
        .all()
    )
...
def clone_selected_reactions(data: CloneRequest, db: Session):
    selected = (
        db.query(Reaction)
        .filter(Reaction.id.in_(data.reaction_ids), Reaction.user_id == None)
        .all()
    )
    for r in selected:
