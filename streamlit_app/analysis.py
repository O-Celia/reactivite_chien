...
def app():
    ...
    def load_data():
        ...
        response = requests.get(f"{API_BASE_URL}/entry/", headers=headers, timeout=60)
    ...
    if (
        "triggers" in df.columns
        and df["triggers"].apply(lambda x: isinstance(x, str)).any()
    ):
    ...
    if periode == "Cette année":
        ...
        selected_year = st.selectbox(
            "Année", list(range(2000, today.year + 1))[::-1], index=0
        )
        ...
        if month_filter:
            ...
            selected_month = st.selectbox(
                "Mois",
                list(range(1, 13)),
                format_func=lambda m: mois_francais_liste[m - 1],
            )
    ...
    df_counts = (
        df_exploded.groupby(["triggers", "severity"]).size().reset_index(name="count")
    )
    ...
    if not df.empty:
        ...
        df_current = df_all[
            df_all["entry_date"].dt.to_period("M") == selected_month_period
        ]
        df_previous = df_all[
            df_all["entry_date"].dt.to_period("M") == previous_month_period
        ]
        ...
        st.subheader(
            f"Proportion d'entrées à haute intensité (≥ 4) pour {mois_fr} (comparé au mois précédent)"
        )
