...
def app():
    ...
    try:
        response = requests.get(
            "http://localhost:8000/entry/", headers=headers, timeout=60
        )
    ...
    for entry in data:
        reaction_text = (
            ", ".join(entry["reactions"])
            if isinstance(entry["reactions"], list)
            else entry["reactions"]
        )
        trigger_text = (
            ", ".join(entry["triggers"])
            if isinstance(entry["triggers"], list)
            else entry["triggers"]
        )
    ...
    available_years = (
        sorted({e["date_obj"].year for e in filtered_data})
        if filtered_data
    ...
    with col1:
        selected_year = st.selectbox(
            "Année", available_years, index=available_years.index(datetime.today().year)
        )
    with col2:
    ...
    with st.expander("Informations supplémentaires sous forme de liste"):
        filtered_events = [
            ...
            if e["date_obj"].month == selected_month_num
            and e["date_obj"].year == selected_year
        ]
