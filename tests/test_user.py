...
def created_user(user_data):
    ...
    if response.status_code not in (200, 201):
        raise Exception(
            f"User creation failed: {response.status_code} - {response.json()}"
        )
    user = response.json()
    yield user
    # Teardown
...
def test_update_me_pass(created_user, user_data):
    token = client.post("/users/login", json=user_data).json()["access_token"]
    update_data = {"username": "montest"}
    response = client.put(
        "/users/me", json=update_data, headers={"Authorization": f"Bearer {token}"}
    )
    try:
        assert response.status_code == 200
        assert response.json()["username"] == "montest"
...
def test_get_me_fail_unauthenticated():
    ...
    try:
        ...
        print(
            f"{GREEN}Test non passant : accès à /me sans authentification rejeté OK{RESET}"
        )
    except AssertionError:
...
def test_delete_me_fail_unauthenticated():
    ...
    try:
        assert response.status_code == 401
        print(
            f"{GREEN}Test non passant : suppression sans authentification rejetée OK{RESET}"
        )
    except AssertionError:
